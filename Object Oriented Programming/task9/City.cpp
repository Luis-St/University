...
namespace lsv9 {
	...
	std::unique_ptr<std::string[]> City::copyStringArray(const std::string *array, const int size) {
	...
	City::City(const lsv9::City &other): City(other.position, other.pois.get(), other.numberOfPOIs) {}
	...
	void City::add(const std::string &name) {
		auto result = copyStringArray(this->pois.get(), this->numberOfPOIs + 1);
		result[this->numberOfPOIs] = name;
		this->numberOfPOIs++;
		this->pois = std::move(result);
	}
	...
	bool City::remove(const std::string &name) {
		...
		if (found) {
			...
			auto result = copyStringArray(this->pois.get(), this->numberOfPOIs);
			this->pois = std::move(result);
		}
		return found;
	}
	...
	City& City::operator=(const City &other) {
		if (this != &other) {
			...
			this->pois = copyStringArray(other.pois.get(), other.numberOfPOIs);
