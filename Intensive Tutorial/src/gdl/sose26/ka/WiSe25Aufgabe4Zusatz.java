...
public class WiSe25Aufgabe4Zusatz {
	...
	public static void f(int[][] p) {
		// b)
		for (int x = 0; x < (p.length / 50); x++) {
			for (int y = 0; y < (p[0].length / 50); y++) {
		...
		// a)
		// So würde man es machen: Math.min(p.length / 50, p[0].length / 50)
		// Für die Klausur, den Wert selbst berechnen und direkt hinterlegen (6 hier stehen lassen)
		for (int i = 0; i < 6; i++) {
