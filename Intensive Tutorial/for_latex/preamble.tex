...
\usepackage{fancyhdr}
% unicode-math funktioniert nur mit lualatex/xelatex und bricht pdflatex ab.
% Da es aktuell nirgends genutzt wird (kein \setmathfont o.ae.), nur unter
% lua/xelatex laden, damit pdflatex (inkl. beramono = Bera Mono) weiter laeuft.
\usepackage{iftex}
\unless\ifPDFTeX
...
% Page header (fancyhdr-based -- see \usepackage{fancyhdr} above).
% NOTE: fancyhdr and scrlayer-scrpage define the same \?head/\?foot commands
% and clash ("Command \chead already defined") on recent TeX distributions.
% Each task/document sets its own \pagestyle{fancy} (tasks) or \pagestyle{empty}
% (documents), so we only need a sane fancyhdr-based default here.
...
\fancyhead[RE,LO]{\leftmark}           % chapter name on the inner edge
\renewcommand{\headrulewidth}{0.4pt}   % separator line beneath the page header
