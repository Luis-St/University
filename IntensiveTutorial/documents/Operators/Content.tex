...
Mit diesen Operatoren kannst du rechnen (plus, minus, mal, geteilt).
...
\subsubsection*{Wichtig bei Division:}
\begin{lstlisting}
...
\subsection*{Beschreibung:}\\
Mit diesen Operatoren kannst du Werte vergleichen. Das Ergebnis ist immer \texttt{true} (wahr) oder \texttt{false} (falsch).
...
\subsubsection*{Wichtig bei Strings:}
\begin{lstlisting}
String text1 = "Hallo";
String text2 = "Hallo";
...
\subsection*{Beschreibung:}\\
Mit diesen Operatoren kannst du mehrere Bedingungen kombinieren (z.B. "wenn A UND B wahr sind").
...
\subsubsection*{Wichtig - Kurzschluss-Auswertung:}
\begin{lstlisting}
// Bei && prueft Java die rechte Seite nicht mehr, wenn die linke schon false ist
int[] zahlen = {1, 2, 3};
int index = 5;
if (index < zahlen.length && zahlen[index] > 0) {
    // Kein Fehler! zahlen[index] wird gar nicht mehr ausgefuehrt,
    // weil index < zahlen.length bereits false ist
}
\end{lstlisting}
...
\subsection*{Beschreibung:}\\
Mit diesen Operatoren kannst du Werte in Variablen speichern oder ändern.
...
\subsection*{Beschreibung:}\\
Mit diesen Operatoren kannst du eine Zahl um 1 erhöhen oder verringern.
...
\subsubsection*{Unterschied: Vorher oder nachher erhöhen:}
\begin{lstlisting}
int a = 5;
int b = a++;  // b = 5, dann a = 6 (++ nachher: erst den Wert nehmen, dann erhoehen)

int c = 5;
int d = ++c;  // c = 6, dann d = 6 (++ vorher: erst erhoehen, dann den Wert nehmen)
\end{lstlisting}
...
\subsection*{Beschreibung:}\\
Der \texttt{+} Operator kann auch Texte zusammenfügen (z.B. "Hallo" + " " + "Welt" ergibt "Hallo Welt").
...
\subsection*{Beschreibung:}\\
Wenn du mehrere Operatoren in einer Zeile hast, werden sie in einer bestimmten Reihenfolge ausgeführt (wie in Mathe: "Punkt vor Strich").

\subsubsection*{Von höchster zu niedrigster Priorität:}
...
\begin{achtung}
	\begin{itemize}
		\item Wenn du dir unsicher bist, setze Klammern! Das macht den Code klarer.
		\item "Punkt vor Strich": Mal und Geteilt kommen vor Plus und Minus
		\item Vergleiche (z.B. \texttt{>}) werden vor logischen Operatoren (z.B. \texttt{\&\&}) ausgeführt
		\item Klammern haben immer Vorrang - nutze sie, um die Reihenfolge festzulegen
	\end{itemize}
\end{achtung}
