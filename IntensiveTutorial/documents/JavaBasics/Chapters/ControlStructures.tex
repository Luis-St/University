...
If-Bedingungen ermöglichen es, Code nur unter bestimmten Voraussetzungen auszuführen.
Sie sind notwendig, wenn das Programm Entscheidungen treffen soll: \("\)Wenn eine Bedingung erfüllt ist, dann führe diese Anweisungen aus.\("\)

\textbf{Typische Anwendungsfälle:}
\begin{itemize}
	\item Prüfen, ob ein Wert einen bestimmten Zustand hat
	\item Vergleichen von Werten
	\item Reagieren auf Benutzereingaben
	\item Fehlerbehandlung (z.B. Index-Prüfung)
\end{itemize}
...
For-Schleifen ermöglichen es, Code mehrfach auszuführen. Sie sind besonders nützlich, wenn man:
\begin{itemize}
	\item Alle Elemente eines Arrays durchlaufen möchte
	\item Alle Zeichen eines Strings prüfen will
	\item Eine Aufgabe eine bestimmte Anzahl wiederholen möchte
	\item Mit Indizes arbeiten muss (z.B. zum Ändern von Array-Werten)
\end{itemize}
...
\textbf{Wichtig:} Alle drei Teile (Initialisierung, Bedingung, Inkrement) sind \textbf{optional} und können weggelassen werden!

\textbf{Typisches Muster für Arrays:}
\begin{lstlisting}
...
\textbf{Typisches Muster für Strings:}
\begin{lstlisting}
...
\textbf{Alle drei Teile können weggelassen werden!}

\textbf{Initialisierung weglassen}
...
While-Schleifen verwendet man, wenn:
\begin{itemize}
	\item Die Anzahl der Wiederholungen im Voraus nicht bekannt ist
	\item Die Schleife bis zum Eintreten einer bestimmten Bedingung laufen soll
	\item Man \("\)so lange wie nötig\("\) wiederholen möchte
	\item Man nach einem bestimmten Element sucht und abbrechen will, wenn es gefunden wurde
\end{itemize}

\textbf{Unterschied zur For-Schleife:}
\begin{itemize}
	\item For-Schleife: \("\)Wiederhole genau n-mal\("\) oder \("\)Durchlaufe alle Elemente\("\)
	\item While-Schleife: \("\)Wiederhole solange diese Bedingung wahr ist\("\)
\end{itemize}
